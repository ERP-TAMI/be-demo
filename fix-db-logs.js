...
async function run() {
  ...
  const { rows } = await client.query("SELECT id, reason, target_label FROM po_version_logs WHERE reason LIKE '%Ã%' OR target_label LIKE '%Ã%'");
  ...
  for (let row of rows) {
    ...
    if (r !== row.reason || l !== row.target_label) {
       await client.query("UPDATE po_version_logs SET reason = $1, target_label = $2 WHERE id = $3", [r, l, row.id]);
    }
  }
