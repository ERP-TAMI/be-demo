...
import { computeRedistribution, RedistributionInput } from './dynamic-target.engine.js';

function makeInput(overrides: Partial<RedistributionInput> = {}): RedistributionInput {
  return {
    plannedQuantity: 500,
    month: 5,
    year: 2026,
...
describe('computeRedistribution', () => {
  it('distributes remaining evenly across unlocked future days', () => {
    const result = computeRedistribution(makeInput({
      dailyRows: [
    ...
    expect(result[0]).toMatchObject({ day: 16, plannedQty: 100, isManualOverride: false });
    expect(result[4]).toMatchObject({ day: 20, plannedQty: 100, isManualOverride: false });
  });
  ...
  it('returns empty array when no future days exist (past plan)', () => {
    const result = computeRedistribution(makeInput({ todayDay: 20, etdDay: 20 }));
  ...
  it('sets targetPerDay to 0 when remaining <= 0 (already done)', () => {
    const result = computeRedistribution(makeInput({
      plannedQuantity: 100,
      dailyRows: [{ day: 10, plannedQty: 100, actualQty: 110, isManualOverride: false }],
    }));
  ...
  it('treats entire month as future when plan is for a future month (todayDay=0)', () => {
    const result = computeRedistribution(makeInput({ todayDay: 0, etdDay: 10 }));
